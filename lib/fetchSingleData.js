  const fs = require('fs')
  const s3 = require('./s3')
  const FetchClass = require('../apis/FetchClass')
  const htmlParser = require('./htmlParser')

  const fetchLandBuild = async (cityCode, townCode, sectCode, landBuild, project, authConfig = {}) => {
    try {
      const {enuid, ensid, cookieValue} = authConfig
      const apis = new FetchClass({enuid, ensid, cookieValue})
